...
def run_calibrate(input_image, lstm, agg_dev):
    #with tf.device(gpu_dev):
    exec_op = input_image
    input_image = input_image.to(agg_dev)
    input_image.cpu().detach().numpy()


def run_forward(input_image, lstm, agg_dev):
#with tf.device(gpu_dev):
    lstm.to(agg_dev)
    input_image = input_image.to(agg_dev)
...
def run_backward(input_image, lstm, agg_dev):
...
def main(input_tensor_shape, cell_type, input_size, hidden_size, num_layers, dtype, n_iter, n_warm, compute_type, enable_xla, agg_placement):
    ...
    for i in range(n_warm):
        compfunc(input_image, lstm, agg_dev)
    print("done")
